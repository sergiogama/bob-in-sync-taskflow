...
export default function TicketDetailPage() {
  ...
  return (
    <>
      <div className="page-header detail-header"><div><div className="breadcrumbs"><Link to="/tickets">Tickets</Link><span>/</span><span>TF-{String(ticket.id).padStart(4, '0')}</span></div><div className="title-with-status"><h1>{ticket.title}</h1><StatusBadge status={ticket.status} /></div><p>Created by {ticket.created_by} on {formatDate(ticket.created_at)}</p></div><Link className="button secondary" to={`/tickets/${ticket.id}/edit`}>Edit ticket</Link></div>
