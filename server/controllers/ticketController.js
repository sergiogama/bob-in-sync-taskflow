export function createTicketController(ticketService) {
  return {
    ...
    create(req, res) { res.status(201).json({ ticket: ticketService.create(req.body, req.user) }); },
    update(req, res) { res.json({ ticket: ticketService.update(Number(req.params.id), req.body, req.user) }); },
    addComment(req, res) {
